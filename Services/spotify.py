...
class TrackFetchWorker(QThread):
    ...
    def run(self):
        try:
            response = self.sp_client.current_user_saved_tracks(limit=self.limit, offset=self.offset, market=self.market)
...
class PlaylistTrackFetchWorker(QThread):
    ...
    def run(self):
        try:
            response = self.sp_client.playlist_tracks(self.playlist_id, limit=self.limit, offset=self.offset, market=self.market)
            self.finished.emit(response['items'], self.offset)
        except Exception as e:
            self.error.emit(str(e), self.offset)
...
class Spotify:
    ...
    def get_playlist_tracks(self, playlist_id, max_workers=5, progress_callback=None):
        ...
        def fetch_batch(offset):
            try:
                response = self.sp.playlist_tracks(playlist_id, limit=50, offset=offset, market=self.market)
                return offset, response['items'], None
            except Exception as e:
                return offset, [], str(e)
        
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(fetch_batch, batch_num * batch_size): batch_num 
    ...
    def get_user_tracks(self, max_workers=5, progress_callback=None):
        ...
        def fetch_batch(offset):
            try:
                response = self.sp.current_user_saved_tracks(limit=50, offset=offset, market=self.market)
                return offset, response['items'], None
            except Exception as e:
                return offset, [], str(e)
