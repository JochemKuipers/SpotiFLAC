...
class DeezerDownloader:
    ...
    def extract_metadata(self, track_data):
        ...
        if "contributors" in track_data:
            ...
            metadata["artists"] = (
                ", ".join(artists) if artists else metadata.get("artist", "")
            )
    ...
    async def download_by_isrc(self, isrc, output_dir="."):
        ...
        print(
            f"Found track: {metadata.get('artists', 'Unknown')} - {metadata.get('title', 'Unknown')}"
        )
        ...
        try:
            ...
            safe_title = "".join(
                c
                for c in metadata.get("title", "Unknown")
                if c.isalnum() or c in (" ", "-", "_")
            ).rstrip()
            safe_artist = "".join(
                c
                for c in metadata.get("artists", "Unknown")
                if c.isalnum() or c in (" ", "-", "_")
            ).rstrip()
            ...
            print(
                f"File size: {downloaded} bytes ({downloaded / (1024 * 1024):.2f} MB)"
            )
