...
class TidalDownloader:
    ...
    def get_available_apis():
        try:
            response = requests.get(
                "https://status.monochrome.tf/api/stream", timeout=10, stream=True
            )
    ...
    def select_api_interactive():
        ...
        while True:
            try:
                choice = input(
                    f"\nSelect API (0-{len(apis)}) [1 for fastest]: "
                ).strip()
    ...
    def search_tracks(self, query):
        try:
            ...
            search_data = requests.get(
                url=search_url, headers=header, timeout=self.timeout
            )
    ...
    def get_track_info(self, query, isrc=None):
        ...
        try:
            ...
            if isrc:
                isrc_items = [
                    item for item in result["items"] if item.get("isrc") == isrc
                ]
            ...
            if not selected_track:
                raise Exception(
                    f"Track not found: {query}" + (f" (ISRC: {isrc})" if isrc else "")
                )
    ...
    def download_file(
        self, url, filepath, is_paused_callback=None, is_stopped_callback=None
    ):
        ...
        while retry_count <= self.max_retries:
            try:
                ...
                print(
                    f"Download error (attempt {retry_count}/{self.max_retries}): {str(e)}"
                )
    ...
    def embed_metadata(self, filepath, track_info, search_info=None):
        try:
            ...
            if search_info and search_info.get("artists"):
                ...
                search_info
                and search_info.get("artist")
                and search_info["artist"].get("name")
            ):
            ...
            audio.save()
            print(
                f"Metadata embedded successfully for: {track_info.get('title', 'Unknown')}"
            )
            return True
    ...
    def download(
        ...
        return self._download_single(
            query, isrc, output_dir, quality, is_paused_callback, is_stopped_callback
        )

    def _download_single(
        self, query, isrc, output_dir, quality, is_paused_callback, is_stopped_callback
    ):
        ...
        track_title = self.sanitize_filename(
            track_info.get("title", f"track_{track_id}")
        )

        output_filename = os.path.join(
            output_dir, f"{artist_name} - {track_title}.flac"
        )
        ...
        if os.path.exists(output_filename):
            ...
            if file_size > 0:
                print(
                    f"File already exists: {output_filename} ({file_size / (1024 * 1024):.2f} MB)"
                )
