...
def generate_totp():
    ...
    try:
        resp = requests.get(
            "https://open.spotify.com/api/server-time", headers=headers, timeout=10
        )
        if resp.status_code != 200:
            raise Exception(
    ...
    if u.netloc == "embed.spotify.com":
        if not u.query:
            raise SpotifyInvalidUrlException(
                "ERROR: url {} is not supported".format(uri)
            )
        qs = parse_qs(u.query)
...
def get_json_from_api(api_url, access_token):
    ...
    if req.status_code != 200:
        raise SpotifyWebsiteParserException(
            f"ERROR: {api_url} gave us not a 200. Instead: {req.status_code}"
        )
...
def get_access_token():
    try:
        ...
        if req.status_code != 200:
            return {
                "error": f"Failed to get access token. Status code: {req.status_code}"
            }
        return req.json()
    except Exception as e:
...
def fetch_tracks_in_batches(
    url: str, access_token: str, batch_size: int = 100, delay: float = 1.0
    ...
    if url_info["type"] == "playlist":
        try:
            playlist_data = get_json_from_api(
                playlist_base_url.format(url_info["id"]), access_token
            )
            ...
            if batch:
                ...
                tracks, num_batches = fetch_tracks_in_batches(
                    tracks_url, access_token, 100, delay
                )
        ...
        try:
            album_data = get_json_from_api(
                album_base_url.format(url_info["id"]), access_token
            )
            if not album_data:
            ...
            if batch:
                ...
                tracks, num_batches = fetch_tracks_in_batches(
                    tracks_url, access_token, 50, delay
                )
                raw_data["tracks"]["items"] = tracks
                raw_data["_batch_count"] = num_batches
                raw_data["_batch_enabled"] = True
        ...
        try:
            track_data = get_json_from_api(
                track_base_url.format(url_info["id"]), access_token
            )
            if not track_data:
        ...
        try:
            artist_data = get_json_from_api(
                artist_base_url.format(url_info["id"]), access_token
            )
            if not artist_data:
            ...
            if batch:
                albums, num_batches = fetch_tracks_in_batches(
                    albums_url, access_token, 50, delay
                )
                raw_data = {
...
def format_album_data(album_data):
        ...
        if track_id and album_data.get("_token"):
            try:
                full_track_data = get_json_from_api(
                    track_base_url.format(track_id), album_data.get("_token")
                )
                if full_track_data:
    ...
    return {"album_info": album_info, "track_list": track_list}


def format_playlist_data(playlist_data):
    image_url = (
        playlist_data.get("images", [{}])[0].get("url", "")
    ...
    return {"playlist_info": playlist_info, "track_list": track_list}
    ...
    for album in albums:
        ...
        if access_token and album.get("id"):
            try:
                ...
                if album_tracks_data:
                    ...
                    for track in tracks:
                        ...
                        if track_id:
                            try:
                                full_track_data = get_json_from_api(
                                    track_base_url.format(track_id), access_token
                                )
                                if full_track_data:
                        ...
                        formatted_track = {
                            ...
                            "release_date": album.get("release_date", ""),
                            "track_number": track.get("track_number", 0),
                            "external_urls": track.get("external_urls", {}).get(
                                "spotify", ""
                            ),
                            "isrc": track_isrc,
                        }
                ...
                print(
                    f"Error getting tracks for album {album.get('name', '')}: {str(e)}"
                )
                continue
...
if __name__ == "__main__":
    ...
    artist_discography_all = (
        "https://open.spotify.com/artist/0du5cEVh5yTK9QJze8zA0C/discography/all"
    )
    artist_discography_albums = (
        "https://open.spotify.com/artist/0du5cEVh5yTK9QJze8zA0C/discography/album"
    )
    artist_discography_singles = (
        "https://open.spotify.com/artist/0du5cEVh5yTK9QJze8zA0C/discography/single"
    )
    artist_discography_compilations = (
        "https://open.spotify.com/artist/0du5cEVh5yTK9QJze8zA0C/discography/compilation"
    )

    print("=== Testing Artist Discography (All) ===")
    filtered_discography = get_filtered_data(
        artist_discography_all, batch=True, delay=0.1
    )
